...
Current method for standardizing the data. Used on solely the velocity data
'''
...
def stdPersonXYZ(data, numberOfTests, timeScores):
	for i in range(0,int(numberOfTests)):
		...
		for j in range (0, 25):
			...
			for k in range (offset2, offset2 + 3*math.floor(maxEntries/2)):
				if (k < offset2 + (timeScores[i]//2)*3):
					if (k%3 == 0):
						data[i][k] = (data[i][k]- np.min(dataPointsX))/(np.max(dataPointsX) - np.min(dataPointsX))
					elif (k%3 == 1):
						data[i][k] = (data[i][k]-np.min(dataPointsY))/(np.max(dataPointsY) - np.min(dataPointsY))
					elif (k%3 == 2):
						data[i][k] = (data[i][k]-np.min(dataPointsZ))/(np.max(dataPointsZ) - np.min(dataPointsZ))
				else:
					k =  offset2 + (timeScores[i]//2)*3
	...
	for i in range( 0, int(numberTests)):
		...
		for j in range (0, 25):
			...
			resultsFileL = open(newDir + "\\Velocity_" + file_names_super[int(j)] + ".csv", "a+")
			for k in range (offset2, offset2 + 3*math.floor(maxEntries/2)):
...
def copyLabels():
	for i in range (0, int(numberTests)):
		...
		resultsFileL = open("D:\\CMU\\CMU_HCII_REU\\ExerciseDetection\\stdData\\test" + str(i) +"\\label.csv", "a+")
