...
def calcMaxEntries():
	...
	for i in range(0,int(numberTests)):
		...
		for line in open(dirname + "\\" + DATA_FOLDER + "\\test" + str(i) + "\\" + FLAGS.source + "_" + file_names_super[0] + '.csv'):
		...
		for l in range(numTimeStamps,numTimeStamps+(timeScores[i])):
			...
			for j in range(0, bodySize):
				if FLAGS.position:
					...
					fp = open(dirname + "\\"+ DATA_FOLDER +"\\test" + str(i)+ "\\Position_" + file_names[j] + '.csv')
				...
				if FLAGS.velocity:
					fp = open(dirname + "\\"+ DATA_FOLDER +"\\test"+ str(i)+ "\\Velocity_" + file_names[j] + '.csv')
