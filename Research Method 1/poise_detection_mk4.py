...
tf.app.flags.DEFINE_string(name='label', default='Test',help='This is the name of the test')
# OLD DATA INPUT CODE #####################################
TRAIN_PERCENT = 0.7
...
def extract_data():
	'''
	data =  np.empty((int(numberTests),27, int(maxEntries),3))
	for i in range(0, int(numberTests)):
		for j in range(0, 27):
			k = 0
			for line in open(dirname + "\\Data\\test" + str(i)+ "\\Position_" + file_names[j]):
...
def run_experiment(argv=None):
	...
	train_input_fn, train_input_hook = get_train_inputs(
		batch_size=10, excercise_data=exercise_data )
	eval_input_fn, eval_input_hook = get_train_inputs(
		batch_size=10, excercise_data=exercise_data )	
...
def architecture(inputs, is_training, scope='ExcersizeConvNet'):
	...
	with tf.variable_scope(scope):
		with slim.arg_scope(
			...
			net = slim.max_pool2d(net,2, stride=4, scope='pool2')
			net = slim.conv2d(net,40, [5,5], padding='VALID', scope='conv3')
			net = slim.max_pool2d(net, 2, stride=4, scope='pool4')
			net = tf.reshape(net, [-1, 312 * 312 * 40])
			...
			net = slim.fully_connected(inputs, 256, scope='dense1')
			net = slim.dropout(net, is_training=is_training, scope='drop2')
			net = slim.fully_connected(net, 256, scope='dense3')
			net = slim.dropout(net, is_training=is_training, scope='drop4')
			net = slim.fully_connected(net, 11, scope='output',activation_fn=None)
...
def get_test_inputs(batch_size, exercise_data):
	...
	def test_inputs():
		...
		with tf.name_scope('Test_data'):
			#Get MINST data
			images = excercise_data[3]
			labels = excercise_data[2]
			#Define Placeholders
			images_placeholder = tf.placeholder(images.dtype, images.shape)
			labels_placeholder = tf.placeholder(labels.dtype, labels.shape)
			#Build Dataset Iterator
			dataset = tf.data.Dataset.from_tensor_slices((images_placeholder, labels_placeholder))
			dataset = datset.batch(batch_size)
			iterator = dataset.make_initializable_iterator()
			next_example, next_label = iterator.get_next()
			#Set runhook to initialize iterator
			iterator_initializer_hook.iterator_initializer_func = \
