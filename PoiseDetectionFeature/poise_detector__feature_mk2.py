...
tf.app.flags.DEFINE_float('threshold',0.3,'threshold for confusion matrix')
...
def oneHot(labels):
	print("start one hot", len(labels))
	'''
...
def extractData():
	...
	'''
	#average
	data =  np.empty((sum(timeScores), 8))
	...
	#for i in range(0, 3):	
		...
		#for l in range(numTimeStamps,3):
			if FLAGS.position:
	...
	'''
	print("extract labels", len(labels))
	print("extract data", len(data))
	return data, labels
...
def partitionData(data, labels):
	...
	minFrames = 1000000
	for i in range(len(distLabels)):
		if (distLabels[i] != 0 and distLabels[i] < minFrames):
			minFrames = distLabels[i]
	trainMin = math.floor(minFrames*.7)

	train = trainMin * 6
	test = sum(timeScores) - train

	trainMinData = np.empty((train, int(8)))
	trainMinLabels = []
	testMinData = np.empty((test, int(8)))
	testMinLabels = []
...
newDir = dirname + '\\Models&Results\\sampleFolder'
...
def main(argv = None):
	...
	with tf.Session() as sess:
		...
		#[0-5: falsely identified as such, 6: unknown]
		confusionMatrix = np.zeros((6,7))

		Total = 0.0
		Accurate = 0.0
		for i in range (0, len(predicList)):
			if predicList[i][1] == predicList[i][2]:
				confusionMatrix[predicList[i][2]][predicList[i][2]] = confusionMatrix[predicList[i][2]][predicList[i][2]] + 1
				Accurate = Accurate + 1
				Total = Total + 1
			elif predicList[i][1] == -1:
				confusionMatrix[predicList[i][2]][6] = confusionMatrix[predicList[i][2]][6] + 1
				Total = Total + 1
			else:
