...
class preprocess:
	...
	def __init__(self, data):
		...
		#calculates distance from Left Foot to Right Foot
		RfootX = (self.data[60])
		RfootY = (self.data[61])
		RfootZ = (self.data[62])
		LfootX = (self.data[69])
		...
		spineBottomY = self.data[13]
		spineBottomZ = self.data[14]
		vectorRFootX = RfootX - spineBottomX
		vectorRFootY = RfootY - spineBottomY
		vectorRFootZ = RfootZ - spineBottomZ
		vectorLFootX = LfootX - spineBottomX
		vectorLFootY = LfootY - spineBottomY
		vectorLFootZ = LfootZ - spineBottomZ

		dotProductRL = vectorRFootX*vectorLFootX + vectorRFootY*vectorLFootY + vectorRFootZ*vectorLFootZ
		magnitudeR = math.sqrt((vectorRFootX * vectorRFootX) + (vectorRFootY * vectorRFootY) + (vectorRFootZ * vectorRFootZ))
		magnitudeL = math.sqrt((vectorLFootX * vectorLFootX) + (vectorLFootY * vectorLFootY) + (vectorLFootZ * vectorLFootZ))
		tempAngle = dotProductRL/(magnitudeR * magnitudeL)
		self.feetAngle = math.acos(tempAngle)

		#calculate angle between hands
		neckX = self.data[3]
		neckY = self.data[4]
		neckZ = self.data[5]
		vectorRHandX = RhandX - neckX
		vectorRHandY = RhandY - neckY
		vectorRHandZ = RhandZ - neckZ
		vectorLHandX = LhandX - neckX
		vectorLHandY = LhandY - neckY
		vectorLHandZ = LhandZ - neckZ

		dotProductRL = vectorRHandX*vectorLHandX + vectorRHandY*vectorLHandY + vectorRHandZ*vectorLHandZ
		magnitudeR = math.sqrt((vectorRHandX * vectorRHandX) + (vectorRHandY * vectorRHandY) + (vectorRHandZ * vectorRHandZ))
		magnitudeL = math.sqrt((vectorLHandX * vectorLHandX) + (vectorLHandY * vectorLHandY) + (vectorLHandZ * vectorLHandZ))
		tempAngle = dotProductRL/(magnitudeR * magnitudeL)
		self.handAngle = math.acos(tempAngle)
	...
	def add_feat(self):
		...
		feat_vec = [self.comX, self.comY, self.comZ, self.wrist_to_ankle, self.chest_to_knee, self.chestX, self.chestY, self.chestZ, self.feetDistance, self.feetAngle, self.handDistance, self.handAngle]
...
def std(features):
	...
	for j in range(0,len(std_feat[0])):
		feature_vector.append([std_feat[0][j],std_feat[1][j],std_feat[2][j],std_feat[3][j],std_feat[4][j],std_feat[5][j],std_feat[6][j],std_feat[7][j],std_feat[8][j],std_feat[9][j],std_feat[10][j],std_feat[11][j]])
